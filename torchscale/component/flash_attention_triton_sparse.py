...
def _fwd_kernel(
    ...
    softmax_scale,
    q_sparse_ratio, kv_sparse_ratio, window_size,
    ...
    off_b = off_hb // nheads
    off_h = off_hb % nheads
    head_stride = (nheads - 1) // kv_sparse_ratio + 1
    ...
    end_n = seqlen_k if not IS_CAUSAL else tl.minimum(((start_m + 1) * BLOCK_M * q_sparse_ratio - window_size) // kv_sparse_ratio, seqlen_k)
    ...
    for start_n in range(0, end_n, BLOCK_N):
        ...
        if IS_CAUSAL: 
            qk += tl.where((offs_m[:, None] * q_sparse_ratio) > ((start_n + offs_n) * kv_sparse_ratio + intercept + window_size)[None, :], 0, float("-inf"))
        if BIAS_TYPE != 'none':
            if BIAS_TYPE == 'vector':
                if EVEN_N:
...
def _bwd_kernel_one_col_block(
    ...
    begin_m = 0 if not IS_CAUSAL else ((((start_n * BLOCK_N) * kv_sparse_ratio + window_size) // q_sparse_ratio)// BLOCK_M) * BLOCK_M
    # triton bug when intercept != 0
    ...
    for start_m in range(begin_m, num_block_m * BLOCK_M, BLOCK_M):
        ...
        # triton bug when intercept != 0
        if IS_CAUSAL:
            # qk = tl.where((offs_m_curr * q_sparse_ratio)[:, None] > ((offs_n * kv_sparse_ratio + intercept + window_size)[None, :]), qk, float("-inf"))
            qk = tl.where((offs_m_curr * q_sparse_ratio)[:, None] > ((offs_n * kv_sparse_ratio + intercept + window_size)[None, :]), qk, float("-inf"))
        if BIAS_TYPE != 'none':
...
def _flash_attn_forward(q, k, v, bias=None, causal=False, softmax_scale=None, q_sparse_ratio=1, kv_sparse_ratio=1, window_size=4096):
    # shape constraints
...
def _flash_attn_backward(do, q, k, v, o, lse, dq, dk, dv, bias=None, causal=False, softmax_scale=None, q_sparse_ratio=1, kv_sparse_ratio=1, window_size=4096):
...
class FlashAttnFunc(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, q, k, v, bias=None, causal=False, softmax_scale=None, q_sparse_ratio=1, kv_sparse_ratio=1, window_size=4096):
        """
        ...
        o, lse, ctx.softmax_scale = _flash_attn_forward(
            q, k, v, bias=bias, causal=causal, softmax_scale=softmax_scale, q_sparse_ratio=q_sparse_ratio, kv_sparse_ratio=kv_sparse_ratio, window_size=window_size,
        )
        ctx.save_for_backward(q, k, v, o, lse, bias)
        ctx.causal = causal
        ctx.q_sparse_ratio = q_sparse_ratio
        ctx.kv_sparse_ratio = kv_sparse_ratio
    ...
    def backward(ctx, do, dlse):
        ...
        with torch.inference_mode():
            ...
            _flash_attn_backward(
                ...
                bias=bias, causal=ctx.causal, softmax_scale=ctx.softmax_scale, q_sparse_ratio=ctx.q_sparse_ratio, kv_sparse_ratio=ctx.kv_sparse_ratio, window_size=ctx.window_size,
            )
